...
Our final list of genes is saved in `prioritized_genes_from_part1.txt`.
...
- Gemini model:
  - Gemini 2.0 flash (Default)
    ...
    - Beware of the Free tier rate limit (should be enough for this project; the prompt generated from 50 PMC fulltext articles is around 500,000 tokens)
...
### 2. Analyze the Retrieved Literature with Gemini

Process all collected PMC publications for the prioritized genes to construct a prompt embedded with the Part 2 instructions for Gemini to conduct a comprehensive analysis as required.
...
- Generate a prompt embedded with the Part 2 instructions
- Call Gemini's API to analyze the organized literature
...
You can also use the `--check`/`-c` flag to just check the generated prompt and the estimated input tokens first without sending the API request, for a sanity check. 
